...
| Tooling | How I use it |
|---|---|
| Codex, gstack/superpowers | 기획, 설계, 구현, review 관점을 분리해 작업 단위를 검토 |
| Obsidian vault | 프로젝트 결정, 트러블슈팅, 세션 로그를 장기 기억으로 유지 |
| Graphify | 코드 구조와 의존성 흐름을 먼저 확인하고 변경 범위 판단 |
| Verification loop | AI 제안은 초안으로 두고, 테스트·렌더 검수·성능 근거로 채택 여부 결정 |
...
<table>
  <tr>
    <td width="50%" valign="top">
      ...
      <p>2인 팀. 영상 업로드부터 AI 분석 결과 스트림까지 백엔드 계약 설계.</p>
      <ul>
        <li>Spring Boot 백엔드와 Python AI worker pipeline 전담</li>
        <li>GCS Signed URL + Redis Streams/Pub/Sub + SSE로 영상 분석 흐름 분리</li>
        <li>추천 피드 HTTP p95 <code>251.233ms -> 122.642ms</code>, cursor aggregate p95 <code>9.794ms</code></li>
      </ul>
      <p>
    ...
    <td width="50%" valign="top">
      ...
      <ul>
        ...
        <li>검색 좌표 <code>47,190개</code>, 카페 <code>3,950개</code>, 리뷰 <code>22,144개</code> 수집</li>
        <li>원천 키워드 <code>80,601개 -> 30,336개</code> 정제 후 추천 입력 데이터로 구조화</li>
      </ul>
      <p>
  ...
  <tr>
    <td width="50%" valign="top">
      ...
      <ul>
        ...
        <li>Redis atomic + queue + async 처리로 동시성 제어</li>
        <li>max response <code>233ms -> 7ms</code>, 평균 응답 시간 <code>40.9%~47.8%</code> 감소</li>
      </ul>
      <p>
...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/k6%20%2F%20JMeter-performance-7D64FF?style=flat-square&logo=k6&logoColor=white" alt="k6 and JMeter" />
</p>
